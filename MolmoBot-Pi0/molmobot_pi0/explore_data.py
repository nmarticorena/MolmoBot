...
def build_group_tree(group: h5py.Group | h5py.File):
    ret = {}
    for key in natural_sort(group.keys()):
        if isinstance(group[key], h5py.Group):
            ret[key] = build_group_tree(group[key])
...
def print_group(args, file: h5py.File):
    if args.prefix is None:
        tree = build_group_tree(file)
    else:
        tree = build_group_tree(file[args.prefix])
...
def print_dataset(args, file: h5py.File):
    ...
    if dataset.dtype == np.uint8:
        if dataset.ndim == 1:
            ...
            buf.write(f"Unsure how to print dataset of shape {dataset.shape} with dtype {dataset.dtype}\n")
    else:
