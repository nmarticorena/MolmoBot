...
class WebsocketPolicyServer:
    ...
    def __init__(
        ...
        assert all(type(p) == type(policies[0]) for p in policies), (
        ...
        self._policy_locks = [asyncio.Lock() for _ in policies]
        self._policy_semaphore = asyncio.Semaphore(len(policies))
    ...
    async def _acquire_policy(self):
        async with self._policy_semaphore:
            for i, lock in enumerate(self._policy_locks):
                if not lock.locked():
                    async with lock:
                        yield self._policies[i]
                    break
            else:
                raise RuntimeError("No policy lock available")
    ...
    async def _handler(self, websocket: _server.ServerConnection):
        ...
        async with self._acquire_policy() as policy:
            logger.info(f"Reset policy for the new connection from {websocket.remote_address}")
            policy.reset()
