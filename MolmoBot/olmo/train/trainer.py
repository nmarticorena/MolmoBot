...
class MixedLoader:
    """Interleave a primary loader with an action-only loader."""

    def __init__(self, primary: DataLoader, action: DataLoader, action_interval: int = 1, action_rate: Optional[float] = None):
    ...
    def __iter__(self):
        ...
        action_iter = iter(self.action)
        step = 0
        rng = np.random.RandomState(0)
        while True:
            if self.action_rate is not None:
                use_action = rng.rand() < self.action_rate
            else:
                use_action = ((step + 1) % (self.action_interval + 1) == 0)
            ...
            if use_action:
                ...
                try:
                    batch = next(primary_iter)
                except StopIteration:
                    primary_iter = iter(self.primary)
                    batch = next(primary_iter)

            # batch["_is_action_batch"] = use_action
            # if use_action:
            #     if "loss_masks" in batch:
            #         batch["loss_masks"] = torch.zeros_like(batch["loss_masks"])
            #     if "labels" in batch:
            #         batch["labels"] = torch.full_like(batch["labels"], -100)
            #     if "response_mask" in batch:
            #         batch["response_mask"] = torch.zeros_like(batch["response_mask"])
            yield batch
            step += 1
try:
...
class Trainer:
    ...
    action_loader: Optional[DataLoader]
    device: torch.device
    evaluators: List[LossDatasetEvaluator]
    inference_evaluators: List[Any]
    ...
    def __post_init__(self):        
        ...
        if self.action_loader is not None:
            log.info(
                f"Interleaving action loader "
                + (f"with probability {self.cfg.action_loader_rate} per batch."
                   if self.cfg.action_loader_rate is not None
                   else f"every {self.cfg.action_batch_interval} batch(es).")
            )
            self.train_loader = MixedLoader(
                primary=self.train_loader,
                action=self.action_loader,
                action_interval=self.cfg.action_batch_interval,
                action_rate=self.cfg.action_loader_rate,
            )
    ...
    def _remove_sharded_checkpoint(self, idx: int, checkpoints: List[Path]):
        ...
        # Check if this checkpoint should be retained based on frequency
        should_retain = False
        if self.cfg.checkpoint_retention_frequency is not None:
            # Extract step number from checkpoint path (format: "step{number}")
            checkpoint_name = Path(oldest_checkpoint).name
            if checkpoint_name.startswith("step"):
                try:
                    step_num = int(checkpoint_name[4:])  # Remove "step" prefix
                    if step_num % self.cfg.checkpoint_retention_frequency == 0:
                        should_retain = True
                except ValueError:
                    # If we can't parse the step number, proceed with normal removal
                    pass

        barrier()
        if get_fs_local_rank() == 0 and not should_retain:
    ...
    def model_forward(
        ...
        with torch.autocast("cuda", dtype=self.cfg.autocast_precision):
            ...
            if self.cfg.response_logits_only:
                ...
                loss_masks = loss_masks[response_mask]
                labels = labels[response_mask]
    ...
    def train_batch(self, batch: Dict[str, Any], compute_metrics) -> Tuple[torch.Tensor, Optional[Dict]]:
        ...
        for micro_batch in micro_batches:
            ...
            # Only Flow loss for action models
    ...
    def train_step(self, batch: Dict[str, Any], compute_metrics: bool = True) -> Dict[str, float]:
        ...
        if self.cfg.model.vision_backbone is not None:
            initial_lr_dict = {
                ...
                "action_expert": self.cfg.optimizer.action_expert_learning_rate,
            }
            for group in self.optim.param_groups:
